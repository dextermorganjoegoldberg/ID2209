...
species Guest skills:[moving]{
	...
	reflex enterTarget when: target != nil and location distance_to(target.location) = 0{
		...
		ask infoCenter at_distance(1) {
			...
			if (myself.isBad and self.guard.target = nil) {
...
species SecurityGuard skills:[moving]{
	...
	reflex moveToTarget when: target != nil {
		...
		if (!dead(target) and location distance_to(target.location) < 1) {
			ask target {
				do die;
			}
			target <- nil;
